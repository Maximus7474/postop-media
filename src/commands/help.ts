	...
	callback: async (logger, client, interaction) => {
		...
		for (const [, cmd] of client.commands) {
			...
			const resolvedPermissions =
				requiredPermissions !== undefined && requiredPermissions !== null
					? BigInt(requiredPermissions)
					: null;

			const hasPermission =
				!resolvedPermissions || memberPermissions.has(resolvedPermissions);

			if (!(commandData.description && hasPermission)) continue;

			const commandName =
				commandData.name_localizations?.[locale] ?? commandData.name;
			const description =
				commandData.description_localizations?.[locale] ??
