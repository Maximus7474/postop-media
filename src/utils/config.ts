...
const CustomUploadSchema = z.object({
  ...
  endpoint: z.string().url({ message: "Post-OP requires a valid URL for custom drop-offs." }),
...
.superRefine((data, ctx) => {
  if (data.UploadMethod === 'custom' && !data.CustomUpload) {
    ctx.addIssue({
      code: 'custom',
      message: "When UploadMethod is 'custom', the 'CustomUpload' settings are mandatory.",
      path: ["CustomUpload"],
    });
  }
});
