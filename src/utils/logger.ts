...
class Logger {
	...
	info(...args: unknown[]): void {
		console.log(
			`${this.timestamp()} ${cyan(`[INFO]   `)} (${this.origin})`,
			...args,
		);
	}
	...
	success(...args: unknown[]): void {
		console.log(
			`${this.timestamp()} ${green(`[SUCCESS]`)} (${this.origin})`,
			...args,
		);
	}
	...
	warn(...args: unknown[]): void {
		console.warn(
			`${this.timestamp()} ${yellow(`[WARN]   `)} (${this.origin})`,
			...args,
		);
	}
	...
	error(...args: unknown[]): void {
		console.error(
			`${this.timestamp()} ${red(`[ERROR]  `)} (${this.origin})`,
			...args,
		);
	}
}
